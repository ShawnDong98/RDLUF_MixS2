...
def shift_3d(inputs,step=2):
    ...
    temp = torch.zeros((bs, nC, row, col)).to(inputs.device)
    for i in range(nC):
        temp[:,i,:,:] = torch.roll(inputs[:,i,:,:], shifts=step*i, dims=2)
    inputs = temp
    return inputs
...
def shift_back_3d(inputs,step=2):
    ...
    temp = torch.zeros((bs, nC, row, col)).to(inputs.device)
    for i in range(nC):
        temp[:,i,:,:] = torch.roll(inputs[:,i,:,:], shifts=(-1)*step*i, dims=2)
    inputs = temp
    return inputs
...
class DUF_MixS2(nn.Module):
    ...
    def forward(self, y, Phi):
        ...
        v_pre = checkpoint(self.head_GD, y, x0, Phi)
        v_pre = shift_back_3d(v_pre)
        log_dict['stage0_v'] = v_pre
        x_pre, enc_outputs, bottolenect_out, dec_outputs = checkpoint(self.head_PM, v_pre)
        x_pre = shift_3d(x_pre)
        log_dict['stage0_x'] = x_pre
       

        for i in range(self.opt.stage-2):
            v_pre = checkpoint(self.body_GD[i], y, x_pre, Phi) if not self.opt.body_share_params else checkpoint(self.body_GD, y, x_pre, Phi)
            v_pre = shift_back_3d(v_pre)
            log_dict[f'stage{i+1}_v'] = v_pre
            x_pre, enc_outputs, bottolenect_out, dec_outputs = checkpoint(self.body_PM[i], v_pre, enc_outputs, bottolenect_out, dec_outputs) if not self.opt.body_share_params else checkpoint(self.body_PM, v_pre, enc_outputs, bottolenect_out, dec_outputs)
            x_pre = shift_3d(x_pre)
            log_dict[f'stage{i+1}_x'] = x_pre
        
        v_pre = checkpoint(self.tail_GD, y, x_pre, Phi)
        v_pre = shift_back_3d(v_pre)
        log_dict[f'stage{self.opt.stage-1}_v'] = v_pre
        out, enc_outputs, bottolenect_out, dec_outputs = checkpoint(self.tail_PM, v_pre, enc_outputs, bottolenect_out, dec_outputs)
