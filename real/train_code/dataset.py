...
class dataset(tud.Dataset):
    ...
    def __getitem__(self, index):
        if self.isTrain == True:
            ...
            hsi = self.HSI[index1]
        shape = np.shape(hsi)
        ph = random.randint(0, shape[0] - self.size)
        pw = random.randint(0, shape[1] - self.size)
        label = hsi[ph:ph + self.size:1, pw:pw + self.size:1, :]
        ...
        meas = np.sum(mask_3d_shift * temp_shift, axis=2)
